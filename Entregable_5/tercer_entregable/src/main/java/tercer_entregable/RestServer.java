...
public class RestServer extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
		...
		vertx.createHttpServer().requestHandler(router::handle).listen(8080, "0.0.0.0", result -> {
			if (result.succeeded()) {
		...
		// ENDPOINT groupId con placa

		router.route("/api/placas/*").handler(BodyHandler.create());
		router.get("/api/placas/:groupId").handler(this::getGroupIdPorPlaca);
		...
		router.get("/api/sensores/group/:groupId").handler(this::getIdGroupSensor);
		...
		router.get("/api/actuadores/group/:groupId").handler(this::getIdGroupActuador);
	...
	private void getGroupIdPorPlaca(RoutingContext rc) {
		...
		SQLClient.getConnection(connection -> {
			...
			if (connection.succeeded()) {
				...
				connection.result().query(
						"SELECT * FROM entregable3.placas where groupId = " + rc.request().getParam("groupId"),
						...
						x -> {
							...
							for (Row r : resultado) {
								...
								a.add(JsonObject.mapFrom(new Placa(r.getInteger("valueId"), r.getInteger("groupId"),
	...
	private void getIdGroupSensor(RoutingContext rc) {
		...
		SQLClient.getConnection(connection -> {
			...
			if (connection.succeeded()) {
				...
				connection.result().query(
						"SELECT * FROM entregable3.sensores where groupId = " + rc.request().getParam("groupId"),
	...
	private void getIdGroupActuador(RoutingContext rc) {
		...
		SQLClient.getConnection(connection -> {
			...
			if (connection.succeeded()) {
				...
				connection.result().query(
						"SELECT * FROM entregable3.actuadores where groupId = " + rc.request().getParam("groupId"),
							...
							for (Row r : resultado) {
								...
								if (f.equals(r.getLocalDateTime("fechaHora"))) {
									...
									threeLastSensors.add(JsonObject.mapFrom(new SensorEntity(r.getInteger("valueId"),
											r.getInteger("groupId"), r.getInteger("placaId"), r.getInteger("sensorId"),
											r.getFloat("valor"),
											r.getLocalDateTime("fechaHora")
													.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
											r.getString("nombre"))));
	...
	private void setNewSensor(RoutingContext rc) {
		...
		SQLClient.getConnection(connection -> {
			...
			if (groupId == null || sensorId == null || placaId == null || value == null || fechaHora == null
					|| nombre == null) {
				...
				SQLClient.preparedQuery(nuevaEntrada, nuevoSensor, x -> {
					...
					if (x.succeeded()) {
						...
						System.out.println("SE HA HECHO UN POST");
						funcionamiento2(sensorId);
					}
	...
	private void deleteOneSensor(RoutingContext rc) {
		...
		SQLClient.getConnection(connection -> {
			...
			if (connection.succeeded()) {
				...
				connection.result().query(
						"DELETE FROM entregable3.sensores where sensorId =" + rc.request().getParam("sensorId"), x -> {
	...
	public static float mapeoLuminosidadBrillo(float valorEntrada) {
		
		return 255 - valorEntrada*255/4095;
		
	}
	

	public void funcionamiento2(Integer sensorId) {
			...
			SQLClient.getConnection(connection -> {
				...
				if (connection.succeeded()) {
					...
					connection.result().preparedQuery(
							"SELECT * FROM Sensores WHERE sensorId = ? ORDER BY valueId DESC LIMIT 1;",

							Tuple.of(sensorId), res -> {

								if (res.succeeded()) {

									RowSet<Row> resultado = res.result();
									List<Float> valores = new ArrayList<>();

									for (Row r : resultado) {

										valores.add(r.getFloat("valor"));
										valores.add(r.getInteger("groupId").floatValue());
									...
									if (!valores.isEmpty()) {
										...
										Float valor = valores.get(0);

										String topic = "Group_" + valores.get(1).intValue();
										...
										payload.put("brillo", mapeoLuminosidadBrillo(valor));
		
										
										//mqttClient.publish(topic, , null, false, false)
										mqttClient.publish(topic, payload.toBuffer(), MqttQoS.AT_LEAST_ONCE, false,
												false);
